...
**Loop config**
- **Steps:** SPEC → IMPLEMENT → REVIEW (reviewer: `code-review`, `evidence-discipline`) → TEST (advanced: `test-runner`, `evidence-discipline`) → FIX (conditional) → ENHANCE (conditional) → LOOP GATE (value-gated, conditional — clean pass exits to EXPORT; unmet ⇒ scoped re-verify) → EXPORT → HUMAN REVIEW → COMMIT.
